...
public class GuacamoleWebSocketHandler extends TextWebSocketHandler {
    ...
    private Map<String, String> extractParams(URI uri) {
        ...
        Arrays.stream(query.split("&")).forEach(pair -> {
            ...
            if (parts.length == 2) {
                queryPairs.put(URLDecoder.decode(parts[0], StandardCharsets.UTF_8),
    ...
    private void closeQuietly(WebSocketSession session, GuacamoleTunnel tunnel) {
        logger.info("Closing WebSocket session and Guacamole tunnel for session ID: {}", session.getId());
        try {
            session.close();
